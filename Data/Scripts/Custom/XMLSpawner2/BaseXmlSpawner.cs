...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
                    ...
                    if (IsSpecialItemKeyword(baseitemtype))
                    {
                        ...
                        switch (kw)
                        {
                            ...
                            case itemKeyword.SCROLL:
                                {
                                    ...
                                    if (itemkeywordargs.Length == 3)
                                    {
										...
										Item item = Loot.RandomScroll(LootPackChange.RandomLootLevel());
                                        if (item != null)
                                        {
                                            pack.DropItem(item);
                                            // could call applyobjectstringproperties on a nested propertylist here to set item attributes
                            ...
                            case itemKeyword.POTION:
                                {
                                    // syntax is POTION
                                    Item item = Loot.RandomPotion(12, true);
                                    if (item != null)
                                    {
                                        pack.DropItem(item);
                                        // could call applyobjectstringproperties on a nested propertylist here to set item attributes
                            ...
                            case itemKeyword.LOOTPACK:
                                {
                                    ...
                                    if (itemkeywordargs.Length == 2)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            lootpack.Generate(m, pack, false, m_KillersLuck, LootPackChange.RandomLootLevel());
                                            ...
                                            if (m != null)
                                                lootpack.Generate(m, pack, true, m_KillersLuck, LootPackChange.RandomLootLevel());
                                        }
                                        catch
                                        {
        ...
        public static Item MagicJewelry(int minLevel, int maxLevel)
        {
			...
			if (item == null)
				return null;
			return LootPackChange.EnchantItem(Utility.RandomMinMax(minLevel, maxLevel), item);
        }
